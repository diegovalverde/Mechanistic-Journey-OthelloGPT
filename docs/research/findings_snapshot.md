...
| Moderate evidence | Local vs averaged transformed semantic directions are related but not identical; the executed notebook reports cosine similarity 0.6178 for the G6 mine-vs-theirs direction. |
...
| D. MLP7 is causally important. | Strong evidence | Supported by layer-7 component attribution and component ablations. |
| E. Specific MLP7 neurons have selective effects. | Weak to moderate evidence | Supported by attribution, group ablation, condition activation, and small end-to-end tests; effects are mixed and often small. |
| F. Specific neurons implement the Othello legality rule. | Open question | Not established by the current notebook. No finding should claim a complete rule implementation by a specific neuron. |
