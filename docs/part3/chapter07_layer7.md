...
It is also useful to look at targets that are not valid captures. The comparison below samples empty illegal targets that have at least one adjacent run of opponent discs, but no friendly terminator at the far end. These are capture-like local patterns that fail the Othello legality rule.

<figure markdown>
![Per-board non-valid capture-like layer sensitivity](../figures/chapter07_nonvalid_capture_line_layer_sensitivity.png)
<figcaption>
Ten sampled non-valid capture-like targets with per-board layer sensitivity. The blue square is the illegal target, gold squares are contiguous opponent runs, and red squares mark the failed or missing terminator. The heatmaps use an invalid-target-vs-legal-logit contrast rather than the legal-move contrast above, so the numbers should be read as a comparison control rather than the same statistic. In this sample, sensitivity to non-valid line-like structure is often strongest around the middle-to-late layers, especially layers `4` and `5`, rather than concentrating at layer `7`.
</figcaption>
</figure>

This contrast helps sharpen the interpretation of the valid-capture result. Layer 7 does not simply become green whenever a board contains an opponent line. The strongest late signal in the valid-capture plot is tied to positions where the line actually supports a legal move. Non-valid line-like patterns can still be salient, but their layer profile is different.
...
The original notebook layer sweep used only layers `2`, `4`, `6`, and `7`. It did not test every block in the reported table. The per-board rasters above extend the same code path to all eight layers for visualization, but they are still lightweight, small-sample runs rather than complete layer-by-layer probe studies.
...
- Chapter 7 measured figures: [chapter07_layer_sweep.svg](../figures/chapter07_layer_sweep.svg), [chapter07_layer7_validation.svg](../figures/chapter07_layer7_validation.svg), [chapter07_capture_line_layer_sensitivity.png](../figures/chapter07_capture_line_layer_sensitivity.png), and [chapter07_nonvalid_capture_line_layer_sensitivity.png](../figures/chapter07_nonvalid_capture_line_layer_sensitivity.png).
