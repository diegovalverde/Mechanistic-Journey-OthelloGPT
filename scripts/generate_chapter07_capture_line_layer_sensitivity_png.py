#!/usr/bin/env python3
"""Generate a Chapter 7 capture-line layer sensitivity PNG.
...
- sample legal mid-game Othello prefixes
- choose legal moves with nontrivial capture lines
- train lightweight board probes for each layer
- score capture-line sensitivity with the notebook's legality contrast
...
def draw_board(ax, position: dict, index: int) -> None:
    ...
    ax.set_title(
        f"{index + 1}. {move_info['label']}  flips={move_info['num_flipped']}  "
        f"lines={move_info['num_capture_lines']}  prefix={position['prefix_len']}",
        loc="left",
        pad=2,
        fontsize=8.5,
        color="#172033",
...
def render_png(
    ...
    ax_left_header.text(0, 0.75, "Valid capture board", ha="left", va="top", fontsize=11, weight="bold", color="#172033")
    ax_left_header.text(0, 0.25, "blue = move, gold = captured, green = terminator", ha="left", va="top", fontsize=7.5, color="#5c667a")
    ...
    ax_right_header.text(
        0.0,
        0.25,
        "Each row is that board's layer ratio; colors share one global scale.",
        ha="left",
        va="top",
        fontsize=7.5,
        color="#5c667a",
    ...
    fig.suptitle("Othello-GPT: per-board capture-line layer sensitivity", x=0.03, y=0.996, ha="left", fontsize=15, weight="bold")
    fig.text(
        0.03,
        0.982,
        "Every board row has its own 8-layer sensitivity heatmap from legality-gradient projections.",
        ha="left",
        va="top",
        fontsize=8,
        color="#5c667a",
...
def serialize_position(position: dict) -> dict:
    ...
    return {
        ...
        "chosen_move": {
            ...
            "num_flipped": int(move_info["num_flipped"]),
            "num_capture_lines": int(move_info["num_capture_lines"]),
        ...
        rows.append(
            {
                ...
                "chosen_move": position["move_info"]["label"],
                "prefix_len": int(position["prefix_len"]),
                "layers": [
                    {
                        "layer": layer_row["layer"],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("othello_capture_layer_sensitivity.png"))
...
def main() -> None:
    ...
    positions = sample_legality_positions(
        num_positions=args.num_positions,
        min_prefix_len=args.min_prefix_len,
        max_prefix_len=args.max_prefix_len,
        seed=LEGALITY_RANDOM_SEED,
    )
    print(f"Sampled {len(positions)} positions with legal nontrivial capture lines.", flush=True)

    scorer = Chapter7Scorer(model=model, device=device, probe_epochs=args.probe_epochs)
    layer_rows, stats_by_layer = scorer.layer_summary(positions=positions, layers=args.layers)
    ...
    payload = {
        ...
        "configuration": {
            ...
            "score": "Mean per-position absolute legality-gradient projection on capture-line opponent plus friendly terminator squares divided by unrelated occupied controls.",
        },
